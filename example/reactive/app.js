...
class AppElements {
  constructor({ input = null, addBtn = null, priorityBtn = null, list = null, footer = null, badge = null, syncStatus = null } = {}) {
    this.input = input;
    this.addBtn = addBtn;
...
async function submitInput(els) {
  const __defers = [];
  let __panic = null;
  try {
    __defers.push(() => { els.input.focus(); });
    let err = validateTodo(els.input.value);
    if (err !== null) {
      return;
    }
...
function togglePriorityMode(els) {
...
async function main() {
  ...
  els.priorityBtn.addEventListener("click", function() {
    togglePriorityMode(els);
  });
  let loadErr = await loadTodos();
  if (loadErr !== null || __len(todosSignal.get()) === 0) {
...
function createAppShell() {
  ...
  return new AppElements({ input: app.querySelector(".todo-input"), addBtn: app.querySelector(".add-btn"), priorityBtn: app.querySelector(".priority-btn"), list: app.querySelector(".todo-list"), footer: app.querySelector(".footer"), badge: app.querySelector(".high-badge"), syncStatus: app.querySelector(".sync-status") });
}
...
function setupReactiveDOM(els) {
  ...
  let footerView = ctx.computed(function() {
    let todos = todosSignal.get();
    if (__len(todos) === 0) {
      return trusted("");
    }
    let s = statsSignal.get();
    let countText = Plural(s.remaining, "task") + " left";
  ...
  ctx.bind(els.footer, footerView, function(v) {
    return v;
  });
  ctx.bind(els.badge, badgeView, function(v) {
    return v;
  });
  ...
  els.list.addEventListener("dragend", function(e) {
    let li = e.target.closest("li");
    if (li !== null) {
