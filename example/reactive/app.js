...
function renderTodoHTML(t) {
	...
	return trusted(
		'<li class="' +
			...
			id +
			'">\n<input type="checkbox" class="todo-cb" data-action="toggle" data-todo-id="' +
			id +
			'"' +
			checked +
			...
			"</span>" +
			badge +
			'<button class="del-btn" data-action="delete" data-todo-id="' +
			id +
			'">✕</button>\n</li>',
	);
}
...
function setupReactiveDOM(els) {
	...
	let footerView = ctx.computed(function () {
		...
		if (s.completed > 0) {
			clearBtn =
				'<button class="clear-btn" data-action="clear-completed">Clear completed (' +
				String(s.completed) +
				")</button>";
		}
		return trusted(
	...
	try {
		__defers.push(() => {
			(function () {
				{
					let r =
						typeof __panic !== "undefined" && __panic !== null
							? (() => {
									const __r = __panic.message ?? String(__panic);
									__panic = null;
									return __r;
								})()
							: null;
...
function addTodo(text, priority) {
	Signals.batch(function () {
		...
		todosSignal.set(
			__append(
				cur,
				new Todo({ id: nextId, text: text, done: false, priority: priority }),
			),
		);
		nextId++;
		return null;
