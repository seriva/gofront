...
// Each function calls cssClass() once — the reactive library generates a unique
// scoped class name, injects a <style> tag, and returns the class.
// All nested selectors use & as a prefix; the library expands & → .className.
// These classes are applied to their container elements in createAppShell().

// cardStyles scopes the glassmorphism card shell + structural dividers.
func cardStyles() string {
    return cssClass(`
        position: relative;
        z-index: 1;
        width: 100%;
...
// headerStyles scopes the header, title row, animated badge, and tag line.
func headerStyles() string {
    return cssClass(`
...
// inputRowStyles scopes the text input and the Add / priority buttons.
func inputRowStyles() string {
    return cssClass(`
        display: flex;
        gap: 8px;
...
// listStyles scopes the todo list, items, checkboxes, badges, and delete buttons.
...
// footerStyles scopes the footer bar: item count, filter buttons, clear button.
...
// syncStatusStyles scopes the saving/saved/error pill in the header.
// &.saving / &.saved / &.error are nested state selectors — the library
// expands & to the generated class name so each state is properly scoped.
func syncStatusStyles() string {
    return cssClass(`
        font-size: .68rem;
        font-weight: 600;
        letter-spacing: .04em;
