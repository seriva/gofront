...
- **Never commit agentic session artifacts.** Checkbox-driven plan files, tool-specific session directories, and scaffold state generated by any agentic tool are session-local only; never commit them. Design docs belong in `docs/vX.Y.Z/<feature>-plan.md` using the narrative format (Goal → Approach → Edge Cases → Test Plan).
